...
function defaultInstagramSettings(): InstagramSettings {
  ...
  return {
    username: usernameMatch ? "@" + usernameMatch[1] : "ثبت نشده",
    pageType: raw.includes("Business") ? "Business" : raw.includes("Creator") ? "Creator" : "ثبت نشده",
    metaConnection: flags.includes("connectionGuide") ? "آماده اتصال رسمی Meta/API؛ تنظیمات را از پنل اینستاگرام وارد کنید" : "ثبت نشده",
    ...
    notes: "برای اتصال واقعی، Webhook و Page Access Token را از Meta تنظیم کنید.",
    pageAccessToken: process.env.INSTAGRAM_PAGE_ACCESS_TOKEN || "",
    instagramAccountId: process.env.INSTAGRAM_ACCOUNT_ID || "",
    webhookVerifyToken: process.env.INSTAGRAM_VERIFY_TOKEN || "igv_33",
...
function defaultSettings(): Settings {
  return {
    businessName: "Hahahaja",
    welcomeMessage: "Jsnsbsbabbsbsb",
    supportContact: "Nsnsnsnansn",
...
function instagramGuideText(settings: Settings) {
  ...
  return "🔌 راهنمای اتصال واقعی اینستاگرام / Meta\n\n" +
    "1) پیج باید Professional / Business یا Creator باشد و به Facebook Page متصل باشد.\n" +
    "2) در Meta Developer App، محصول Webhooks و Instagram/Messenger را فعال کنید.\n" +
    "3) Callback URL زیر را وارد کنید:\n" + callbackUrl + "\n\n" +
    "4) Verify Token زیر را وارد کنید:\n" + settings.instagram.webhookVerifyToken + "\n\n" +
    "5) Page Access Token را از همین پنل در بخش Page Access Token ثبت کنید.\n\n" +
    "بعد از اتصال، پیام‌های دایرکت و کامنت‌هایی که Meta به webhook بفرستد ذخیره می‌شوند، بر اساس کلمات کلیدی پاسخ می‌گیرند و برای ادمین‌های ربات گزارش می‌شوند.";
}
